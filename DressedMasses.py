...
def SolveMasses(V, plot=False):
    #ADD T=0 HANDLING!! 
    TRange = np.linspace(0,V.fSIGMA*1.25,num=100)
    sigmaRange = np.linspace(0.01, V.fSIGMA*1.25,num=100)
    ...
    Residuals = np.zeros((len(sigmaRange),len(sigmaRange)))
    ...
    for i,T in enumerate(TRange):
        for j,sigma in enumerate(sigmaRange):
            ...
            initial_guess = [V.mSq['Sig'][0](sigma) + (T**2/24)*(3*V.lambdas - (V.c*V.detPow/V.F)*(V.detPow*V.F-1)*(V.detPow*V.F-3)*(V.detPow*V.F-3)*sigma**(V.detPow*V.F-4)), 
                             V.mSq['Eta'][0](sigma) + (T**2/24)*(V.lambdas + (V.c*V.detPow/V.F)*(V.detPow*V.F-1)*(V.detPow*V.F-3)*(V.detPow*V.F-3)*sigma**(V.detPow*V.F-4)),
                              V.mSq['X'][0](sigma)+ (T**2/24)*(V.lambdas + 2*V.lambdaa + (V.c*V.detPow/V.F)*(V.detPow*V.F-3)*(V.detPow*V.F-3)*sigma**(V.detPow*V.F-4)),
                               V.mSq['Pi'][0](sigma) + (T**2/24)*(V.lambdas - (V.c*V.detPow/V.F)*(V.detPow*V.F-3)*(V.detPow*V.F-3)*sigma**(V.detPow*V.F-4))]


            sol = root(bagEquations, initial_guess, method='hybr', tol=0.01)
            Residuals[i,j]=np.sqrt(np.mean((bagEquations.lhs-bagEquations.rhs)**2)) #RMS
            if sol.success and Residuals[i,j]<V.fSIGMA:
                ...
                if plot:
                    print(f"Root finding did not converge for T={T} and sigma={sigma}")
                    print(sol.message)
                MSqSigData[i,j]=None
                MSqEtaData[i,j]=None
                MSqXData[i,j]=None
    ...
    dressedMasses = {
        #Sigma Mass	
        'Sig': interpolate.RectBivariateSpline(TRange, sigmaRange, _MSqSigData,s=4*V.fSIGMA**3,ky=2,kx=2),
		#Eta Prime Mass
        'Eta': interpolate.RectBivariateSpline(TRange, sigmaRange, _MSqEtaData,s=4*V.fSIGMA**3,ky=2,kx=2),
		#X Mass
		'X': interpolate.RectBivariateSpline(TRange, sigmaRange, _MSqXData,s=4*V.fSIGMA**3,ky=2,kx=2),
		#Pi Mass
		'Pi':  interpolate.RectBivariateSpline(TRange, sigmaRange, _MSqPiData,s=4*V.fSIGMA**3,ky=2,kx=2)
		}
    ...
    if plot:
        ...
        im0 = ax[0,0].contourf(X/V.fSIGMA, Y/V.fSIGMA, MSqSigData)
        ...
        im1 = ax[0,1].contourf(X/V.fSIGMA, Y/V.fSIGMA, MSqEtaData)
        ...
        im2 = ax[1,0].contourf(X/V.fSIGMA, Y/V.fSIGMA, MSqXData)
        ...
        im3 = ax[1,1].contourf(X/V.fSIGMA, Y/V.fSIGMA, MSqPiData)
        ...
        im0 = ax.contourf(X/V.fSIGMA, Y/V.fSIGMA, Residuals)
        cbar = plt.colorbar(im0)
        cbar.set_label(r'Residuals $[MeV^2]$',fontsize=14)
        ax.set_xlabel(r'Temperature $T/f_\pi$',fontsize=15)
        ax.set_ylabel(r'$\sigma/f_\pi$',fontsize=15)
        fig.suptitle(f"$f_\pi={V.fSIGMA}$")
        plt.show()
    
    return dressedMasses
...
if __name__ == "__main__":
    ...
    '''m2Sigs = np.array([399.390])
    m2Etas = np.array([671.520])
    m2Xs = np.array([696.639])
    fPis = np.array([90*np.sqrt(3/2)])'''
    
    m2Sigs = np.linspace(3E2**2, 1E3**2/np.sqrt(2), num=3)
    m2Etas = np.linspace(1E2**2, 2E3**2, num=3)
	
    fPis = np.linspace(500, 2000, num=3)
    m2Xs = np.linspace(500**2, 2000**2, num=3)
		
    (m2, c, ls, la)=Potential2.masses_to_lagrangian(m2Sigs[0],m2Etas[0],m2Xs[0],fPis[0],3,6,1/3)
    print(f'm^2={m2}, c={c}, ls={ls}, la={la}')  

    m2=46666.666666666664
    c=4.096e-09
    ls=0.06186666666666667
    la=0.15786666666666666
    N=3
    F=6
    p=1


    V = Potential2.Potential(m2, c, ls, la, N, F, p)

    
    '''
    T=10
    plt.plot(np.linspace(-5,90*1.25,num=100),V.Vtot(np.linspace(-5,90*1.25,num=100),T)-V.Vtot(0,T),label=f"T={T}")
    plt.show()'''
    

    
    grd = SolveMasses(V, plot=True)
